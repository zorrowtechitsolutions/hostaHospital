// src/components/Settings/Map.jsx - With toast notifications
...
const Map = () => {
  ...
  const handleLocationSelect = (lat, lng, addressText) => {
    ...
    showSuccessToast(`Location selected: ${lat}, ${lng}`, 3000);
  ...
  const getCurrentLocation = () => {
    ...
    if (!navigator.geolocation) {
      ...
      showErrorToast('Geolocation is not supported by your browser', 3000);
      setTimeout(() => setLocationStatus(''), 3000);
      return;
    }
        ...
        if (window.google && window.google.maps && window.google.maps.Geocoder) {
          ...
          geocoder.geocode({ location: { lat, lng } }, (results, status) => {
            if (status === 'OK' && results && results[0]) {
              ...
              showSuccessToast('Current location detected successfully!', 3000);
            }
          });
        } else {
          showSuccessToast(`Current location: ${lat}, ${lng}`, 3000);
        }
  ...
  const handleSaveLocation = () => {
    if (!latitude || !longitude) {
      showWarningToast('Please select a location before saving', 3000);
      return;
    }
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-gray-900 font-mono text-lg">{savedLocation.lat}, {savedLocation.lng}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <label className="block text-sm font-medium text-gray-500 mb-1">Last Updated</label>
                    <p className="text-gray-900">{savedLocation.lastUpdated || formatDate(savedLocation.timestamp)}</p>
                  </div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <label className="block text-sm font-medium text-gray-500 mb-1">Full Address</label>
                  <p className="text-gray-900 leading-relaxed">{savedLocation.address || 'No address saved'}</p>
                </div>
              </div>
