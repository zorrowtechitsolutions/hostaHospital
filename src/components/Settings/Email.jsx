...
{hospital_name}`,
  ...
  const handleSave = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      showSuccessToast(
        `"${editingTemplate}" template has been updated successfully!`,
        4000,
        {
          'Template': editingTemplate,
          'Subject': editForm.subject.substring(0, 50) + (editForm.subject.length > 50 ? '...' : '')
        }
      );
  ...
  const handleResetToDefault = useCallback(() => {
    ...
    showSuccessToast(
      `"${activeTemplate}" template has been reset to default!`,
      4000,
      {
              ...
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentTemplate.tags.map(tag => (
                    <code 
                      key={tag} 
                      className="px-2 py-1 bg-white rounded text-sm text-[#1C62A0] border border-blue-200 cursor-pointer hover:bg-gray-100 transition-colors"
                      onClick={() => {
                        setEditForm(prev => ({
                          ...prev,
                          body: prev.body + tag
