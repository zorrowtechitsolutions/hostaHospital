// src/components/superadmin/usermanagement/HospitalAddUser.jsx
...
const HospitalAddUser = () => {
  const { hospitalId } = useParams();
  const location = useLocation();
  const navigate = useNavigate();
  const { hospitalName, userType: initialUserType } = location.state || {};
  ...
  const [formData, setFormData] = useState({ roleType: selectedRoleType, roleId: '' });
  const [errors, setErrors] = useState({});

  // Fetch doctors and staff for this specific hospital only
  ...
  // Fetch roles from API - same pattern as AddNewUser
  const {
    data: rolesData,
  ...
  const rolesList = [
    ...
    ...(rolesData?.data || []).filter(role => (role.hospitalId) === parseInt(hospitalId))
  ];
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div className="text-xs text-gray-500">
            <span className="text-gray-700">{hospitalName}</span>
        ...
        <p className="text-sm text-gray-500 mt-1">Assign roles to doctors or staff members in {hospitalName}</p>
      </div>
...
export default HospitalAddUser;