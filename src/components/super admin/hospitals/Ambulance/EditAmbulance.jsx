// src/components/super-admin/ambulance/EditAmbulance.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate, useLocation, useParams } from 'react-router-dom';
import { ArrowLeft, Truck, Phone, MapPin, Building, Hash } from 'lucide-react';
import { Card, Button, Input } from '../../../ui';
import { Country, State, City } from 'country-state-city';
import { showSuccessToast, showErrorToast } from '../../../ui/Toast';
import { useUpdateAmbulanceMutation, useGetAmbulanceQuery } from '../../../../../app/service/ambulance';
...
const EditAmbulance = () => {
  ...
  const { id } = useParams();
  const ambulanceFromState = location.state?.ambulance;
  const hospitalId = location.state?.hospitalId;

  const [updateAmbulance, { isLoading: isSubmitting }] = useUpdateAmbulanceMutation();
  
  // Fetch ambulance data if not provided via state
  const { data: ambulanceData, isLoading: isLoadingAmbulance } = useGetAmbulanceQuery(
    { id },
    { skip: !!ambulanceFromState }
  );
  ...
  // Populate form when ambulance data is available
  useEffect(() => {
    const ambulance = ambulanceFromState || ambulanceData?.data;
    ...
    if (ambulance) {
      setFormData({
        ...
        address: {
          ...
          state: ambulance.address?.state || '',
          district: ambulance.address?.district || '',
          place: ambulance.address?.place || '',
          pincode: ambulance.address?.pincode || ''
        },
      ...
      if (foundCountry) {
        ...
        const foundState = State.getStatesOfCountry(foundCountry.isoCode).find(s => s.name === ambulance.address?.state);
        if (foundState) {
          setStateCode(foundState.isoCode);
        }
      }
    }
  }, [ambulanceFromState, ambulanceData, countries]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showSuccessToast(`${formData.serviceName} has been updated successfully!`, 3000);
      
      // Navigate back to the appropriate list
      if (hospitalId) {
        navigate(`/super-admin/hospital/ambulances/${hospitalId}`);
      } else {
        navigate('/super-admin/ambulance');
      }
    } catch (error) {
      ...
      <Card>
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Edit Ambulance</h2>
              <p className="text-sm text-gray-500 mt-0.5">Update ambulance service information</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border-t pt-6">
              <h3 className="text-md font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <MapPin className="w-5 h-5 text-[#1C62A0]" />
                Address Information
              </h3>
