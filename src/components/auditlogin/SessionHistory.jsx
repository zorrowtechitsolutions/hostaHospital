...
import { useGetSessionHistoryQuery } from "../../../app/service/audit";
...
// Remove the static AUDIT_ROLES constant since we'll use dynamic data
...
const SessionHistory = () => {
  ...
  // Fetch data with backend filters - simplified search logic
  const {
    ...
    // Search: only for searching name/details
  ...
  // Get unique values for role filter from current page data (dynamic)
  const roles = useMemo(
    () =>
  ...
  const handleExport = () => {
    ...
    try {
      ...
      showSuccessToast(
        `Successfully exported ${exportData.length} sessions to Excel!`,
        3000
      );
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] p-6 font-sans">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-800">Session History</h1>
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm mb-6 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {/* Role Filter - Now using dynamic data */}
